...
export function CreatorTable({ creators, selectedIds, onToggleSelect, onRowClick, sortState, onSort, listId, onPriceUpdate }: Props) {
  ...
  const handleSavePrice = async (creatorId: number) => {
    ...
    // Parse as integer, or set to null if empty/invalid
    const numericPrice = trimmed === '' ? null : parseInt(trimmed, 10);
    
    // Validate it's a valid number (or null)
    ...
    // Save to database if listId is provided
    if (listId) {
      try {
        await listsClient.updateCreatorPrice(listId, String(creatorId), numericPrice);
        // Notify parent component about the price update
        if (onPriceUpdate) {
          onPriceUpdate(String(creatorId), numericPrice);
        }
      } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center h-64 text-gray-400 py-8">
        <div className="text-center">
          <div className="text-lg mb-2">No creators in this list yet</div>
  ...
  return (
    <div className="w-full overflow-x-auto lg:overflow-x-visible">
      <div className="min-w-[1000px] lg:min-w-[1100px] xl:min-w-0">
        <div className="gap-2 sm:gap-3 lg:gap-4 px-3 py-2 bg-[#374151] rounded-t-lg border-b border-[#4b5563] text-[10px] sm:text-xs lg:text-[13px] xl:text-[14px] font-medium text-[#d1d5db] grid grid-cols-[40px_160px_70px_90px_90px_90px_120px_100px_80px_40px] lg:grid-cols-[50px_180px_80px_100px_100px_100px_120px_100px_80px_50px] xl:grid-cols-[50px_2fr_0.7fr_1fr_1fr_1fr_1fr_1fr_0.8fr_50px]">
          <div></div>
          ...
          {onSort ? (
            ...
            <div className="flex items-center gap-1 sm:gap-2 justify-center"><span className="truncate"><span className="hidden md:inline lg:inline xl:hidden">Avg. Views</span><span className="md:hidden lg:hidden xl:inline">Average Views</span></span></div>
          )}
          {/* Engagement - Sortable */}
          {onSort ? (
          ...
          {creators.map((creator, index) => (
            <div key={creator.id} className={`gap-2 sm:gap-3 lg:gap-4 px-3 py-3 items-center hover:bg-[#374151] transition-colors cursor-pointer ${index !== creators.length - 1 ? 'border-b border-[#374151]' : ''} ${selectedIds.has(creator.id) ? 'border-l-4 border-l-[#94c4fc]' : ''} grid grid-cols-[40px_160px_70px_90px_90px_90px_120px_100px_80px_40px] lg:grid-cols-[50px_180px_80px_100px_100px_100px_120px_100px_80px_50px] xl:grid-cols-[50px_2fr_0.7fr_1fr_1fr_1fr_1fr_1fr_0.8fr_50px]`} onClick={() => onRowClick(creator)}>
              ...
              <div className="text-center text-xs lg:text-[13px] xl:text-[13px] font-medium text-[#f8f9fa]">
                ...
                <div className="flex items-center justify-center gap-1 mt-1">
                  {((creator.average_views_change ?? 0) === 0) ? (
                    <span className="mr-0.5 text-gray-500">-</span>
                  ) : (
                    <Icon name={(creator.average_views_change ?? 0) > 0 ? 'PositiveChangeIcon.svg' : 'NegativeChangeIcon.svg'} className="w-2 h-2 sm:w-3 sm:h-3 lg:w-3 lg:h-3 flex-shrink-0" alt={(creator.average_views_change ?? 0) > 0 ? 'Positive change' : 'Negative change'} />
                  )}
                  <span className={`text-[10px] lg:text-[11px] xl:text-[11px] font-medium ${((creator.average_views_change ?? 0) === 0) ? 'text-gray-500' : (creator.average_views_change ?? 0) > 0 ? 'text-[#1ad598]' : 'text-[#ea3a3d]'}`}>{Math.abs(creator.average_views_change ?? 0).toFixed(2)}%</span>
                </div>
              </div>

              <div className="text-center">
                <div className="text-[#f8f9fa] font-medium text-xs lg:text-[13px] xl:text-[13px]">{creator.engagement_rate.toFixed(2)}%</div>
                <div className="flex items-center justify-center gap-1 mt-1">
                  {((creator.engagement_rate_change ?? 0) === 0) ? (
                    <span className="mr-0.5 text-gray-500">-</span>
                  ) : (
                    <Icon name={(creator.engagement_rate_change ?? 0) > 0 ? 'PositiveChangeIcon.svg' : 'NegativeChangeIcon.svg'} className="w-2 h-2 sm:w-3 sm:h-3 lg:w-3 lg:h-3 flex-shrink-0" alt={(creator.engagement_rate_change ?? 0) > 0 ? 'Positive change' : 'Negative change'} />
                  )}
                  <span className={`text-[10px] lg:text-[11px] xl:text-[11px] font-medium ${((creator.engagement_rate_change ?? 0) === 0) ? 'text-gray-500' : (creator.engagement_rate_change ?? 0) > 0 ? 'text-[#1ad598]' : 'text-[#ea3a3d]'}`}>{Math.abs(creator.engagement_rate_change ?? 0).toFixed(2)}%</span>
                </div>
              </div>

              <div className="flex flex-col gap-0.5 min-w-0">
                <div className="flex items-center"><div className="px-[4px] lg:px-[6px] xl:px-[8px] py-[3px] lg:py-[4px] xl:py-[5px] rounded-[10px] bg-[rgba(30,58,138,0.5)] border border-solid border-[#3B82F6] flex items-center"><span className="font-inter font-medium text-[10px] lg:text-[11px] xl:text-[12px] text-[#F9FAFB] leading-none">{creator.primary_niche}</span></div></div>
                <div className="flex items-center"><div className="px-[4px] lg:px-[6px] xl:px-[8px] py-[3px] lg:py-[4px] xl:py-[5px] rounded-[10px] bg-[rgba(20,83,45,0.5)] border border-solid border-[#22C55E] flex items-center"><span className="font-inter font-medium text-[10px] lg:text-[11px] xl:text-[12px] text-[#F9FAFB] leading-none">{creator.secondary_niche}</span></div></div>
              </div>
              ...
              <div className="flex justify-center">
                <div className="flex items-center justify-center w-full max-w-[70px] xl:max-w-none">
