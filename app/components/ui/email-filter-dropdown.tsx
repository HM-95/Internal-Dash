...
interface EmailFilterDropdownProps {
  isOpen: boolean;
  onClose: () => void;
  selectedEmailFilter: 'all' | 'email' | 'no-email';
  onEmailFilterToggle: () => void;
  onReset: () => void;
  onConfirm: () => void;
  triggerRef: React.RefObject<HTMLElement>;
}

// Define the email filter options
const EMAIL_OPTIONS = [
...
export const EmailFilterDropdown: React.FC<EmailFilterDropdownProps> = ({
  isOpen,
  onClose,
  selectedEmailFilter,
  onEmailFilterToggle,
  ...
  useEffect(() => {
    if (isOpen && dropdownRef.current && triggerRef.current) {
      ...
      const dropdownHeight = 200; // Smaller height for fewer options
  ...
  return (
    ...
    >
      <div className="p-3 sm:p-4">
        ...
        {/* Email options list */}
        <div className="max-h-[120px] sm:max-h-[140px] lg:max-h-[160px] overflow-y-auto">
            ...
            {EMAIL_OPTIONS.map((option) => (
              <div
                ...
                className={`flex items-center space-x-2 sm:space-x-3 p-2 rounded-[6px] cursor-pointer transition-colors ${
                  selectedEmailFilter === option.value
                    ? 'bg-blue-900/20 hover:bg-blue-900/30'
                    : 'hover:bg-[#374151]'
                }`}
                onClick={() => {
                  if (option.value === 'all') {
                    // Cycle through: all -> email -> no-email -> all
                    if (selectedEmailFilter === 'all') {
                      // This shouldn't happen since we're already on 'all', but just in case
                      return;
                    }
                  }
                  onEmailFilterToggle();
